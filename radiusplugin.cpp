...
extern "C" {
	...
	OPENVPN_PLUGIN_DEF openvpn_plugin_handle_t OPENVPN_PLUGIN_FUNC(openvpn_plugin_open_v2)(unsigned int *type_mask, const char *argv[], const char *envp[],
		...
		if (string_array_len(argv) > base_parms) {
			...
			if (strncmp(name_value_list.data[0].name, "-c", 2) == 0) {
				...
				if (context->radiusconf.parseConfigFile(name_value_list.data[0].value) != 0 or context->conf.parseConfigFile(name_value_list.data[0].value)
						!= 0) {
			...
			if (context->radiusconf.parseConfigFile("/etc/openvpn/radiusplugin.cnf") != 0 or context->conf.parseConfigFile("/etc/openvpn/radiusplugin.cnf")
					!= 0) {
				cerr << getTime() << "RADIUS-PLUGIN: Bad config file or error in config.\n";
				goto error;
			}
		...
		if (context->conf.getAccountingOnly() == false) {
			*type_mask = OPENVPN_PLUGIN_MASK ( OPENVPN_PLUGIN_AUTH_USER_PASS_VERIFY ) | OPENVPN_PLUGIN_MASK ( OPENVPN_PLUGIN_CLIENT_CONNECT )
	...
	OPENVPN_PLUGIN_DEF int OPENVPN_PLUGIN_FUNC(openvpn_plugin_func_v2)(openvpn_plugin_handle_t handle, const int type, const char *argv[], const char *envp[],
		...
		if (type == OPENVPN_PLUGIN_CLIENT_CONNECT && context->acctsocketbackgr.getSocket() >= 0) {
			...
			try {
				...
				if (DEBUG ( context->getVerbosity() ))
					cerr << getTime() << "RADIUS-PLUGIN: FOREGROUND: Set FramedIP to the IP (" << newuser->getFramedIp() << ") OpenVPN assigned to the user "
							<< newuser->getUsername() << "\n";
				...
				if (newuser != NULL && newuser->isAccounted() == false && newuser->isAuthenticated()) {
					...
					if (DEBUG ( context->getVerbosity() ))
						cerr << getTime() << "RADIUS-PLUGIN: FOREGROUND: Add user for accounting: username: " << newuser->getUsername() << ", commonname: "
...
void * auth_user_pass_verify(void * c) {
	...
	while (!context->getStopThread()) {
		...
		if (DEBUG ( context->getVerbosity() ))
			cerr << getTime() << "RADIUS-PLUGIN: FOREGROUND THREAD: New user: username: " << newuser->getUsername() << ", password: *****" << ", newuser ip: "
					<< newuser->getCallingStationId() << ", newuser port: " << newuser->getUntrustedPort() << " ." << endl;
		...
		if (newuser->getUsername().size() > 0) //&& olduser==NULL)
		{
			...
			if (status == RESPONSE_SUCCEEDED) {
				...
				if (DEBUG ( context->getVerbosity() ))
					cerr << getTime() << "RADIUS-PLUGIN: FOREGROUND THREAD: Receive acctinteriminterval " << newuser->getAcctInterimInterval()
							<< " sec from backgroundprocess." << endl;
